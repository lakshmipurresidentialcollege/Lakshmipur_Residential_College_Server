...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@collegecluster.g6ahyc5.mongodb.net/?retryWrites=true&w=majority&appName=CollegeCluster`;
...
async function run() {
    try {
        ...
        const teachersCollection = client.db('Lakshmipur_Residential_College').collection('teachers');
        const collegeLocationCollection = client.db('Lakshmipur_Residential_College').collection('college_location');
        const userRequestCollection = client.db('Lakshmipur_Residential_College').collection('user_request');
        const teachersCodeCollection = client.db('Lakshmipur_Residential_College').collection('teachers_code');
        const additionalMovementRequestCollection = client.db('Lakshmipur_Residential_College').collection('additional_movement_request');
        const collegeTransectionsCollection = client.db('Lakshmipur_Residential_College').collection('college_transections');
...
// ---------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        const airTicketClientCornerCollection = client.db('Lakshmipur_Residential_College').collection('air_ticket_client_corner');
        ...
        app.post('/shop_code', async (req, res) => {
            ...
            try {
                const existing = await teachersCodeCollection.findOne({});
                if (existing) {
                    await teachersCodeCollection.updateOne(
                        { _id: existing._id },
                        { $set: { shop_code: updatedCode.shop_code } },
                        options
                    );
                } else {
                    await teachersCodeCollection.insertOne({ shop_code: updatedCode.shop_code });
                }
        ...
        app.put('/additional_request_approve/:uid', async (req, res) => {
            ...
            try {
                const result = await teachersCollection.updateOne(filter, movementStatus, options);
                res.send(result);
            } catch (err) {
                res.status(500).send({ error: 'Update failed', details: err });
            }
        ...
        app.get("/staffs", async (req, res) => {
            const staffs = await teachersCollection.find().toArray();
            res.send(staffs);
        });
        app.get("/staff/:id", async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) };
            const staff = await teachersCollection.find(query).toArray();
            if (staff) {
                res.send(staff);
            }
            else {
        ...
        app.get('/staff/uid_query/:uid', async (req, res) => {
            const uid = req.params.uid;
            try {
                const result = await teachersCollection.findOne({ uid: uid });
                if (result) {
                    res.send(result);
                }
                else {
        ...
        app.post('/staff', async (req, res) => {
            ...
            try {
                const result = await teachersCollection.insertOne(newStaff);
                res.send(result);
            } catch (err) {
        ...
        app.post('/new_staff', async (req, res) => {
            ...
            const result = teachersCollection.insertOne(new_staff);
            res.send(result);
        })
        app.put('/staffs_daily_time/:id', async (req, res) => {
            ...
            let attendance
            if (updatedTime.name == 'today_enter_time') {
                attendance = {
                    $set: {
                        today_enter_time: updatedTime.clickedTime,
                        today_date: updatedTime.today_date
                    }
                };
            }
            else if (updatedTime.name == 'today_exit_time') {
                attendance = {
                    $set: {
                        today_exit_time: updatedTime.clickedTime
                    }
                };
            }

            try {
                const result = await teachersCollection.updateOne(filter, attendance, options);
                res.send(result);
            } catch (err) {
                res.status(500).send({ error: 'Update failed', details: err });
            }
        ...
        app.get('/shop_location', async (req, res) => {
            try {
                const location = await collegeLocationCollection.findOne({});
                if (!location) {
        ...
        app.post('/shop_location', async (req, res) => {
            ...
            try {
                const existing = await collegeLocationCollection.findOne({});
                if (existing) {
                    await collegeLocationCollection.updateOne(
                        { _id: existing._id },
                        { $set: { latitude, longitude, shop_range } }
                    );
                } else {
                    await collegeLocationCollection.insertOne({ latitude, longitude, shop_range });
                }
        ...
        app.put('/submit_work_time/:id', async (req, res) => {
            ...
            const todaySummary = {
                ...
                today_enter_time: bodyData.today_enter_time,
                today_exit_time: bodyData.today_exit_time,
            ...
            try {
                ...
                if (bodyData.today_exit_time === '') {
                    await teachersCollection.updateOne(filter, ErrorDoc, { upsert: true });
                    res.send({ message: 'Work time submitted successfully' });
                }
                else {
                    await teachersCollection.updateOne(filter, updateDoc, { upsert: true });
                    res.send({ message: 'Work time submitted successfully' });
                }
            } catch (err) {
        ...
        app.put('/additional_movement_submit/:id', async (req, res) => {
            ...
            try {
                ...
                if (bodyData.additional_enter_time === '') {
                    const result = await teachersCollection.updateOne(filter, ErrorDoc);
                    res.send(result);
                }
                else {
        ...
        app.put('/transection_details/:id', async (req, res) => {
            ...
            try {
                const staff = await teachersCollection.findOne(filter);

                // Step 1: If length > 19, remove first transection
                if (staff?.transections?.length > 19) {
                    await teachersCollection.updateOne(filter, { $pop: { transections: -1 } }); // remove first
                }
        ...
        app.put('/closing_month/:id', async (req, res) => {
            ...
            try {
                ...
                if (staff?.income_history?.length > 12) {
                    await teachersCollection.updateOne(filter, { $pop: { income_history: -1 } }); // remove first
                }
        ...
        app.post('/shop_transections_closing_month', async (req, res) => {
            const bodyData = req.body;
            try {
                const existing = await collegeTransectionsCollection.findOne({});
                if (existing) {
                    await collegeTransectionsSummaryCollection.insertOne(bodyData);
                    await collegeTransectionsCollection.updateOne(
                        { _id: existing._id },
                    ...
                    await collegeTransectionsCollection.insertOne(bodyData);
                }
        ...
        app.put('/start_new_month', async (req, res) => {
            ...
            try {
                const existing = await collegeTransectionsCollection.findOne({});
                const result = await collegeTransectionsCollection.updateOne(
                    { _id: existing._id },
        ...
        app.put('/set_user_category/:uid', async (req, res) => {
            ...
            try {
                const result = await teachersCollection.updateOne(filter, newUserCategory, options);
                res.send(result);
            } catch (err) {
                res.status(500).send({ error: 'Update failed', details: err });
            }
        ...
        app.get('/shop_transections', async (req, res) => {
            const result = await collegeTransectionsCollection.find().toArray();
            res.send(result);
        });
        app.put('/shop_transections', async (req, res) => {
            const filter = await collegeTransectionsCollection.findOne({});
            const bodyData = req.body;
            const transection = {
                transection_id: bodyData.transection_id,
            ...
            if (bodyData.transection_type === 'revenue') {
                ...
                const result = await collegeTransectionsCollection.updateOne(filter, updateDoc);
                res.send(result);
            }
            else {
                const updateDoc = {
        ...
        app.get('/shop_transections_summary', async (req, res) => {
            const result = await collegeTransectionsSummaryCollection.find().toArray();
            res.send(result);
        });
        ...
        app.get('/client_corner', async (req, res) => {
            const result = await studentCornerCollection.find().toArray();
            res.send(result);
        });
        app.get('/client_details/:id', async (req, res) => {
            const id = req.params.id;
            const filter = { _id: new ObjectId(id) };
            const result = await studentCornerCollection.findOne(filter);
            res.send(result);
        });
        app.delete('/client/:id', async (req, res) => {
            const id = req.params.id;
            const filter = { _id: new ObjectId(id) };
            const result = await studentCornerCollection.deleteOne(filter);
            res.send(result);
        });
        app.post('/new_client', async (req, res) => {
            const clientData = req.body;
            const result = await studentCornerCollection.insertOne(clientData);
            res.send(result);
        })
        ...
        app.put('/new_voucher/:id', async (req, res) => {
            ...
            try {
                const client = await studentCornerCollection.findOne(filter);

                // Step 1: If length > 19, remove first transection
                if (client?.vouchers?.length > 10) {
                    await studentCornerCollection.updateOne(filter, { $pop: { vouchers: -1 } }); // remove first
                }
                const result = await studentCornerCollection.updateOne(
                    filter,
        ...
        app.put('/take_payment/:id', async (req, res) => {
            ...
            try {
                const client = await studentCornerCollection.findOne(clientFilter);
                console.log(client.transections.length)
                // Step 1: If length > 19, remove first transection
                if (client?.transections?.length > 15) {
                    await studentCornerCollection.updateOne(filter, { $pop: { transections: -1 } }); // remove first
                }
                const result = await studentCornerCollection.updateOne(
                    filter,
                    {
        ...
        app.put('/change_time/:id', async (req, res) => {
            ...
            const filter = { _id: new ObjectId(id) };
            const bodyData = req.body;
            if (bodyData.name === 'today_enter_time') {
                const result = await teachersCollection.updateOne(filter, {
                    $set: {
                        today_enter_time: bodyData.time,
                    }
                });
                res.send(result);
            }
            if (bodyData.name === 'today_exit_time') {
                const result = await teachersCollection.updateOne(filter, {
                    $set: {
                        today_exit_time: bodyData.time,
                    }
                });
                res.send(result);
            }
        ...
        app.patch('/staff/remove_attendance/:id', async (req, res) => {
            ...
            try {
                const staff = await teachersCollection.findOne({ _id: new ObjectId(id) });
